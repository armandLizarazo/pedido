...
import os
from PIL import Image # Solo Image de Pillow es necesario ahora para abrir y opcionalmente redimensionar
...
class ConfirmDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, title, message):
        ...
        ctk.CTkLabel(self, text=message, wraplength=380, font=("Arial", 12), justify="left").pack(pady=20, padx=10)

        frame_buttons = ctk.CTkFrame(self)
        frame_buttons.pack(pady=10)

        btn_yes = ctk.CTkButton(frame_buttons, text="Sí", command=self._on_yes, width=80)
...
class ChoiceDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, title, message, choices):
        ...
        ctk.CTkLabel(self, text=message, wraplength=460, font=("Arial", 12), justify="left").pack(pady=15, padx=10)

        frame_buttons = ctk.CTkFrame(self)
        frame_buttons.pack(pady=10)

        for text, value in choices:
            btn = ctk.CTkButton(frame_buttons, text=text, command=lambda v=value: self._on_choice(v), width=400) 
...
class SuggestionsPopup(ctk.CTkToplevel):
    ...
    def show_suggestions(self, suggestions, x, y, height):
        ...
        for suggestion in suggestions[:10]: 
            btn = ctk.CTkButton(self.scrollable_frame, text=suggestion,
...
class GestorPedidosApp(ctk.CTk):
    def __init__(self):
        ...
        try:
            ...
            if not os.path.exists(logo_path):
                raise FileNotFoundError(f"El archivo del logo no se encontró en: {logo_path}")

            # Abrir la imagen con Pillow
            logo_image_pil = Image.open(logo_path)
            
            # Definir el tamaño deseado para el logo en la UI
            ...
            logo_image_pil_resized = logo_image_pil.resize((new_width, new_height), resampling_filter)

            self.logo_ctk_image = ctk.CTkImage(light_image=logo_image_pil_resized,
                                              dark_image=logo_image_pil_resized, 
                                              size=(new_width, new_height))
            
            logo_label = ctk.CTkLabel(self.frame_logo, image=self.logo_ctk_image, text="")
            logo_label.pack(pady=10) 

        except FileNotFoundError as fnf_error: 
            print(f"ADVERTENCIA AL CARGAR LOGO (FileNotFoundError): {fnf_error}")
            ctk.CTkLabel(self.frame_logo, text="Logo no encontrado", font=("Arial", 14)).pack(pady=20)
        except Exception as e: 
            print(f"ERROR DETALLADO AL CARGAR LOGO: {e}") 
            print(f"Tipo de error: {type(e)}")
            ctk.CTkLabel(self.frame_logo, text="Error al cargar Logo", font=("Arial", 14)).pack(pady=20)
        ...
        self.frame_superior_entradas.grid(row=0, column=0, columnspan=2, pady=(10,0), sticky="ew") 
        ...
        ctk.CTkLabel(self.frame_superior_entradas, text="Descripción:", font=("Arial", 13)).grid(row=0, column=0, padx=(5,0), pady=(5,0), sticky="sw")
        self.entry_descripcion = ctk.CTkEntry(self.frame_superior_entradas, font=("Arial", 12))
        ...
        self.entry_descripcion.bind("<KeyRelease>", self._actualizar_sugerencias_descripcion)
        self.entry_descripcion.bind("<FocusOut>", lambda e: self.after(200, self._hide_suggestions_if_not_focused_popup)) 

        self.suggestions_popup = SuggestionsPopup(self, self.entry_descripcion)

        ctk.CTkLabel(self.frame_superior_entradas, text="Cantidad:", font=("Arial", 13)).grid(row=0, column=1, padx=(5,0), pady=(5,0), sticky="sw")
        self.entry_cantidad_pedido = ctk.CTkEntry(self.frame_superior_entradas, width=100, font=("Arial", 12))
        self.entry_cantidad_pedido.grid(row=1, column=1, padx=5, pady=(0,5), sticky="ew")
        self.entry_cantidad_pedido.bind("<Return>", self.procesar_elemento_enter)

        ctk.CTkLabel(self.frame_superior_entradas, text="Archivo Pedido:", font=("Arial", 13)).grid(row=0, column=2, padx=(5,0), pady=(5,0), sticky="sw")
        self.opciones_pedido = ["pdcentro", "pdpr", "pdst", "np"]
        self.combo_nombre_pedido = ctk.CTkComboBox(self.frame_superior_entradas, values=self.opciones_pedido, font=("Arial", 12), width=150)
        ...
        self.btn_procesar = ctk.CTkButton(app_content_frame, text="Procesar Elemento", command=self.procesar_elemento, font=("Arial", 14), height=35)
        ...
        self.frame_mensajes.grid(row=3, column=0, columnspan=2, pady=10, padx=5, sticky="ew")
        app_content_frame.grid_rowconfigure(3, weight=0) 


        ctk.CTkLabel(self.frame_bodega, text="Inventario Bodega (bodegac.txt)", font=("Arial", 16, "bold")).pack(pady=5)
        self.texto_bodega = ctk.CTkTextbox(self.frame_bodega, font=("Courier New", 12), wrap="none")
        self.texto_bodega.pack(pady=5, padx=5, fill="both", expand=True)
        self.btn_refrescar_bodega = ctk.CTkButton(self.frame_bodega, text="Refrescar Bodega", command=self.cargar_y_mostrar_bodega_con_sugerencias, font=("Arial", 12))
        self.btn_refrescar_bodega.pack(pady=5)
        
        ctk.CTkLabel(self.frame_pedido, text="Detalle del Pedido", font=("Arial", 16, "bold")).pack(pady=5)
        self.texto_pedido_actual = ctk.CTkTextbox(self.frame_pedido, font=("Courier New", 12), wrap="none")
        ...
        self.btn_cargar_pedido = ctk.CTkButton(frame_botones_pedido, text="Cargar/Ver Pedido", command=self.cargar_y_mostrar_pedido, font=("Arial", 12))
        self.btn_cargar_pedido.pack(side="left", padx=5)
        self.btn_limpiar_pedido_display = ctk.CTkButton(frame_botones_pedido, text="Limpiar Vista Pedido", command=lambda: self.texto_pedido_actual.delete("1.0", ctk.END), font=("Arial", 12))
        self.btn_limpiar_pedido_display.pack(side="left", padx=5)

        ctk.CTkLabel(self.frame_mensajes, text="Mensajes:", font=("Arial", 14)).pack(anchor="w", pady=(5,0))
        self.texto_mensajes = ctk.CTkTextbox(self.frame_mensajes, height=100, font=("Arial", 12), wrap="word") 
        self.texto_mensajes.pack(pady=5, fill="x", expand=True)
        self.texto_mensajes.configure(state="disabled") 

        # CAMBIO DE COLOR DEL BOTÓN SALIR
        self.btn_salir = ctk.CTkButton(main_container, text="Salir", command=self.quit_app, 
                                       fg_color="#FF69B4", hover_color="#D4509A", # Mismos colores que el logo y un hover más oscuro
                                       font=("Arial", 14))
    ...
    def _cargar_items_bodega_para_sugerencias(self):
        ...
        if lineas_bodega:
            ...
            self.lista_items_bodega_sugerencias = sorted(list(set(self.lista_items_bodega_sugerencias)))
    ...
    def _actualizar_sugerencias_descripcion(self, event=None):
        ...
        sugerencias_filtradas = [
            item for item in self.lista_items_bodega_sugerencias 
    ...
    def _hide_suggestions_if_not_focused_popup(self):
        ...
        if focused_widget != self.suggestions_popup and focused_widget != self.entry_descripcion and \
           not (isinstance(focused_widget, ctk.CTkButton) and focused_widget.master == self.suggestions_popup.scrollable_frame):
            self.suggestions_popup.hide()
    ...
    def _inicializar_bodega(self):
        if not os.path.exists(self.archivo_bodega):
            print(f"Advertencia: El archivo '{self.archivo_bodega}' no existe. Se creará uno vacío.")
            try:
                with open(self.archivo_bodega, "w", encoding=self.encoding_archivos) as bodega:
    ...
    def escribir_archivo(self, ruta_archivo, lineas_lista, encabezado_opcional=None):
        try:
            with open(ruta_archivo, "w", encoding=self.encoding_archivos) as f_out:
                ...
                elif encabezado_opcional and os.path.exists(ruta_archivo) and os.path.getsize(ruta_archivo) == 0: 
                    f_out.write(encabezado_opcional)

                f_out.writelines(lineas_lista)
            return True
        except IOError as e:
            self.mostrar_mensaje(f"Error crítico al escribir en '{ruta_archivo}': {e}.", "error")
            return False
    ...
    def leer_archivo_completo(self, ruta_archivo):
        try:
            ...
            self.mostrar_mensaje(f"Error de codificación al leer '{ruta_archivo}'. Verifique.", "error")
            return None 
        except IOError as e:
    ...
    def cargar_y_mostrar_bodega(self):
        ...
        if lineas is not None: 
            if lineas:
                ...
                self.texto_bodega.insert(ctk.END, f"# {self.archivo_bodega} está vacío o no existe.\n")
        else: 
             self.texto_bodega.insert(ctk.END, f"# Error al cargar '{self.archivo_bodega}'.\n")
        self.texto_bodega.configure(state="disabled")

    def cargar_y_mostrar_pedido(self):
        nombre_base = self.combo_nombre_pedido.get() 
        if not nombre_base:
            self.mostrar_mensaje("Seleccione un archivo de pedido de la lista.", "warning")
            return
        ...
        if lineas is not None:
            if lineas: 
                self.texto_pedido_actual.insert(ctk.END, f"# Contenido de: {nombre_pedido_completo}\n")
                for linea in lineas:
                    self.texto_pedido_actual.insert(ctk.END, linea)
            elif os.path.exists(nombre_pedido_completo): 
                 self.texto_pedido_actual.insert(ctk.END, f"# Archivo '{nombre_pedido_completo}' está vacío.\n")
            else: 
                self.texto_pedido_actual.insert(ctk.END, f"# Archivo '{nombre_pedido_completo}' no existe. Se creará al agregar elementos.\n")
        else: 
            self.texto_pedido_actual.insert(ctk.END, f"# Error al cargar '{nombre_pedido_completo}'.\n")
    ...
    def mostrar_mensaje(self, mensaje, tipo="info"):
        ...
        elif tipo == "warning": prefix = "ADVERTENCIA: "
        else: prefix = "INFO: "
    ...
    def procesar_elemento(self):
        ...
        if not descripcion:
            self.mostrar_mensaje("La descripción del elemento no puede estar vacía.", "warning")
        ...
        try:
            ...
            if cantidad_pedido <= 0:
                self.mostrar_mensaje("La cantidad para el pedido debe ser > 0.", "warning")
        ...
        self.mostrar_mensaje(f"Procesando: '{descripcion}', Cant. Pedido: {cantidad_pedido}", "info")

        elementos_bodega_actuales = self.leer_archivo_completo(self.archivo_bodega)
        if elementos_bodega_actuales is None:
            self.mostrar_mensaje("No se pudo leer bodega. Operación cancelada.", "error")
            return
        ...
        for i, linea in enumerate(elementos_bodega_actuales):
            linea_stripped = linea.strip()
            if not linea_stripped or linea_stripped.startswith('#'): continue
        ...
        if not elemento_en_bodega:
            dialog = ConfirmDialog(self, "Agregar a Bodega", f"El elemento '{descripcion}' no existe en bodega.\n¿Desea agregarlo con cantidad 0?")
            if dialog.get_result():
                nueva_linea_bodega = f"    {descripcion} 0\n"
                elementos_bodega_actuales.append(nueva_linea_bodega)
                if self.escribir_archivo(self.archivo_bodega, elementos_bodega_actuales):
                    self.mostrar_mensaje(f"'{descripcion}' agregado a bodega con cantidad 0.", "info")
                    self.cargar_y_mostrar_bodega_con_sugerencias() 
                    cantidad_bodega_actual_item = 0
                    elemento_en_bodega = True 
                    indice_bodega_item = len(elementos_bodega_actuales) - 1
                else:
                    self.mostrar_mensaje(f"No se pudo agregar '{descripcion}' a bodega.", "error")
                    return 
            else:
                self.mostrar_mensaje(f"'{descripcion}' no fue agregado a bodega. No se puede procesar para pedido.", "info")
                return
        else:
            self.mostrar_mensaje(f"'{descripcion}' en bodega. Stock: {cantidad_bodega_actual_item}.", "info")

        if elemento_en_bodega and cantidad_bodega_actual_item > 0:
            if cantidad_pedido > cantidad_bodega_actual_item:
                 self.mostrar_mensaje(f"Pedido ({cantidad_pedido}) para '{descripcion}' excede stock ({cantidad_bodega_actual_item}).", "warning")

            dialog_restar = ctk.CTkInputDialog(text=f"Stock de '{descripcion}': {cantidad_bodega_actual_item}.\n¿Cuántas unidades restar de bodega? (0-{cantidad_bodega_actual_item})\n(Enter o cancelar para no restar)", title="Restar de Bodega")
                ...
                try:
                    ...
                    if 0 <= cantidad_a_restar_de_bodega <= cantidad_bodega_actual_item:
                        if cantidad_a_restar_de_bodega > 0:
                            nueva_cantidad_bodega = cantidad_bodega_actual_item - cantidad_a_restar_de_bodega
                            desc_original_item_bodega = " ".join(elementos_bodega_actuales[indice_bodega_item].strip().split()[:-1])
                            elementos_bodega_actuales[indice_bodega_item] = f"    {desc_original_item_bodega} {nueva_cantidad_bodega}\n"
                            if self.escribir_archivo(self.archivo_bodega, elementos_bodega_actuales):
                                self.mostrar_mensaje(f"Se restaron {cantidad_a_restar_de_bodega} de '{descripcion}'. Stock: {nueva_cantidad_bodega}.", "info")
                                self.cargar_y_mostrar_bodega_con_sugerencias() 
                            else:
                                self.mostrar_mensaje(f"Error al actualizar stock de '{descripcion}'.", "error")
                                return 
                        else:
                            self.mostrar_mensaje(f"No se restaron unidades de '{descripcion}' de bodega.", "info")
                    else:
                        self.mostrar_mensaje(f"Cantidad a restar inválida. Debe ser entre 0 y {cantidad_bodega_actual_item}.", "error")
                except ValueError:
                    self.mostrar_mensaje("Entrada para restar de bodega no es un número válido.", "error")
            else: 
                self.mostrar_mensaje(f"No se modificó el stock de '{descripcion}' en bodega.", "info")
        
        elif elemento_en_bodega and cantidad_bodega_actual_item == 0:
             self.mostrar_mensaje(f"'{descripcion}' tiene 0 stock en bodega. No se puede restar.", "info")
        ...
        if lineas_pedido_actual is None:
            self.mostrar_mensaje(f"Error al leer pedido '{nombre_pedido_completo}'. Operación cancelada.", "error")
            return
        ...
        if elemento_en_pedido_actual:
            self.mostrar_mensaje(f"'{descripcion}' ya existe en '{nombre_pedido_completo}' (Cant: {cantidad_en_pedido_item_actual}).", "info")
            choices = [
                (f"Reemplazar con {cantidad_pedido}", "reemplazar"),
                (f"Sumar {cantidad_pedido} (Total: {cantidad_en_pedido_item_actual + cantidad_pedido})", "sumar"),
                ("No hacer nada", "nada")
            ]
            choice_dialog = ChoiceDialog(self, "Modificar Pedido", f"'{descripcion}' ya está en el pedido '{nombre_pedido_base}'.\nCantidad actual: {cantidad_en_pedido_item_actual}\nCantidad nueva: {cantidad_pedido}", choices)
            decision = choice_dialog.get_result()

            if decision == "reemplazar":
                desc_original_pedido = " ".join(lineas_pedido_actual[indice_pedido_item_actual].strip().split()[:-1])
                lineas_pedido_actual[indice_pedido_item_actual] = f"    {desc_original_pedido} {cantidad_pedido}\n"
                accion_pedido_realizada = f"reemplazado con {cantidad_pedido}"
            elif decision == "sumar":
                desc_original_pedido = " ".join(lineas_pedido_actual[indice_pedido_item_actual].strip().split()[:-1])
                nueva_cantidad_en_pedido = cantidad_en_pedido_item_actual + cantidad_pedido
                lineas_pedido_actual[indice_pedido_item_actual] = f"    {desc_original_pedido} {nueva_cantidad_en_pedido}\n"
                accion_pedido_realizada = f"sumado, nueva cant: {nueva_cantidad_en_pedido}"
            elif decision == "nada":
                self.mostrar_mensaje(f"No se modificó '{descripcion}' en el pedido.", "info")
            else: 
                self.mostrar_mensaje(f"Modificación de '{descripcion}' en pedido cancelada.", "info")
        else: 
        ...
        if accion_pedido_realizada: 
            ...
            if self.escribir_archivo(nombre_pedido_completo, lineas_pedido_actual, encabezado_opcional=encabezado):
                self.mostrar_mensaje(f"'{descripcion}' {accion_pedido_realizada} en '{nombre_pedido_completo}'.", "info")
                self.cargar_y_mostrar_pedido() 
            else:
                self.mostrar_mensaje(f"Error al escribir en '{nombre_pedido_completo}'.", "error")
