...
def cargar_datos(ruta_archivo):
    ...
    try:
        ...
        messagebox.showerror("Error", f"El archivo no fue encontrado en la ruta:\n{ruta_archivo}")
        return None
...
def aplicar_filtros(df, fecha_inicio, fecha_fin, palabra_clave, producto_seleccionado):
    ...
    if fecha_inicio:
        df_filtrado = df_filtrado[df_filtrado['Timestamp'] >= pd.to_datetime(fecha_inicio)]
        print(f"  - Después de filtro 'Fecha Inicio' (>= {fecha_inicio}): {len(df_filtrado)} filas")
    if fecha_fin:
        df_filtrado = df_filtrado[df_filtrado['Timestamp'] < pd.to_datetime(fecha_fin) + pd.Timedelta(days=1)]
        print(f"  - Después de filtro 'Fecha Fin' (<= {fecha_fin}): {len(df_filtrado)} filas")
        
    if palabra_clave:
        df_filtrado = df_filtrado[df_filtrado['Descripcion'].str.contains(palabra_clave, case=False, na=False)]
        print(f"  - Después de filtro 'Palabra Clave' ('{palabra_clave}'): {len(df_filtrado)} filas")
    ...
    if producto_seleccionado and producto_seleccionado != "Todos":
        ...
        print(f"  - Después de filtro 'Producto' ('{producto_seleccionado}'): {len(df_filtrado)} filas")
        
    return df_filtrado

def generar_reporte_agregado(df, agrupar_por, valores, funcion_agregacion, orden, ordenar_por):
    """Genera un reporte agregado (tabla dinámica) con ordenamiento flexible."""
    if df is None or df.empty:
        return None
    mapa_funciones = {'Suma': 'sum', 'Promedio': 'mean', 'Conteo': 'count', 'Máximo': 'max', 'Mínimo': 'min'}
    ...
    try:
        ...
        tabla_dinamica_ordenada = tabla_dinamica.sort_values(by=columna_a_ordenar, ascending=ascending_bool)
...
class AnalizadorVentasApp(tk.Tk):
    def __init__(self):
        ...
        self.reporte_actual = None # Variable para guardar el reporte generado
    ...
    def _crear_widgets(self):
        ...
        ttk.Button(controles_frame, text="Cargar Otro Archivo CSV", command=self.cargar_archivo).grid(row=0, column=0, padx=5, pady=5, sticky="ew")
        self.lbl_archivo = ttk.Label(controles_frame, text=f"Archivo: {self.ruta_archivo.split('/')[-1]}", font=("Arial", 8))
        ...
        ttk.Label(filtros_frame, text="Fecha Inicio:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ...
        ttk.Label(filtros_frame, text="Fecha Fin:").grid(row=0, column=2, padx=5, pady=5, sticky="w")
        ...
        ttk.Label(filtros_frame, text="Palabra Clave (Búsqueda):").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        ...
        ttk.Label(filtros_frame, text="Seleccionar Producto (Opcional):").grid(row=1, column=2, padx=5, pady=5, sticky="w")
        ...
        reporte_frame = ttk.LabelFrame(main_frame, text="Configuración del Reporte", padding="10")
        ...
        self.combo_agrupar = ttk.Combobox(reporte_frame, values=opciones_agrupar, state="readonly")
        self.combo_agrupar.set('Descripcion')
        self.combo_agrupar.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(reporte_frame, text="Calcular:").grid(row=0, column=2, padx=5, pady=5, sticky="w")
        self.combo_valores = ttk.Combobox(reporte_frame, values=opciones_valores, state="readonly")
        self.combo_valores.set('Ganancia')
        self.combo_valores.grid(row=0, column=3, padx=5, pady=5)

        ttk.Label(reporte_frame, text="con la operación:").grid(row=0, column=4, padx=5, pady=5, sticky="w")
        self.combo_funcion = ttk.Combobox(reporte_frame, values=opciones_funcion, state="readonly")
        self.combo_funcion.set('Suma')
        self.combo_funcion.grid(row=0, column=5, padx=5, pady=5)

        ttk.Label(reporte_frame, text="Ordenar por:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.ordenar_por_var = tk.StringVar(value="Valor")
        ttk.Radiobutton(reporte_frame, text="Valor", variable=self.ordenar_por_var, value="Valor").grid(row=1, column=1, padx=5, pady=5, sticky="w")
        ttk.Radiobutton(reporte_frame, text="Nombre", variable=self.ordenar_por_var, value="Nombre").grid(row=1, column=2, padx=5, pady=5, sticky="w")
        
        ttk.Label(reporte_frame, text="en orden:").grid(row=1, column=3, padx=5, pady=5, sticky="w")
        self.combo_orden = ttk.Combobox(reporte_frame, values=['Descendente', 'Ascendente'], state="readonly", width=12)
        self.combo_orden.set('Descendente')
        self.combo_orden.grid(row=1, column=4, padx=5, pady=5)

        ttk.Button(reporte_frame, text="📊 Generar Reporte", command=self.generar_reporte).grid(row=0, column=6, rowspan=2, padx=20, pady=5, sticky="e")
        ...
        ttk.Button(acciones_frame, text="📋 Copiar Descripción", command=self.copiar_descripcion).pack(side=tk.LEFT, padx=(0, 10))
        # --- NUEVO BOTÓN EXPORTAR ---
        ttk.Button(acciones_frame, text="💾 Exportar a .txt", command=self.exportar_txt).pack(side=tk.LEFT)

        resumen_frame = ttk.LabelFrame(main_frame, text="Resumen del Reporte", padding="10")
        resumen_frame.pack(fill=tk.X, pady=(5,0))

        style = ttk.Style()
        style.configure("Resumen.TLabel", font=("Arial", 10, "bold"))
        ttk.Label(resumen_frame, textvariable=self.total_items_var, style="Resumen.TLabel").pack(side=tk.LEFT, padx=15)
        ttk.Label(resumen_frame, textvariable=self.total_cantidad_var, style="Resumen.TLabel").pack(side=tk.LEFT, padx=15)
        ttk.Label(resumen_frame, textvariable=self.total_venta_var, style="Resumen.TLabel").pack(side=tk.LEFT, padx=15)
        ttk.Label(resumen_frame, textvariable=self.total_ganancia_var, style="Resumen.TLabel").pack(side=tk.LEFT, padx=15)
    ...
    def cargar_archivo_inicial(self):
        ...
        try:
            ...
            if self.datos_originales is not None and not self.datos_originales.empty:
                messagebox.showinfo("Éxito", f"Archivo '{self.ruta_archivo}' cargado.\nSe encontraron {len(self.datos_originales)} filas válidas.")
    ...
    def cargar_archivo(self):
        ...
        if self.datos_originales is not None and not self.datos_originales.empty:
            messagebox.showinfo("Éxito", f"Archivo cargado y procesado.\nSe encontraron {len(self.datos_originales)} filas válidas.")
            self.actualizar_lista_productos()
        elif self.datos_originales is not None: 
            messagebox.showwarning("Archivo Vacío", "El archivo fue cargado, pero no se encontraron filas con datos válidos tras la limpieza.")

    def actualizar_lista_productos(self, event=None):
        if self.datos_originales is None: return
        df_filtrado_fecha = self.datos_originales.copy()
        try:
            fecha_inicio = self.cal_inicio.get_date()
            df_filtrado_fecha = df_filtrado_fecha[df_filtrado_fecha['Timestamp'] >= pd.to_datetime(fecha_inicio)]
        except (tk.TclError, TypeError): pass
        try:
            fecha_fin = self.cal_fin.get_date()
            df_filtrado_fecha = df_filtrado_fecha[df_filtrado_fecha['Timestamp'] < pd.to_datetime(fecha_fin) + pd.Timedelta(days=1)]
        except (tk.TclError, TypeError): pass
        
        if 'Descripcion' in df_filtrado_fecha:
            productos_unicos = sorted(df_filtrado_fecha['Descripcion'].dropna().unique().tolist())
            self.combo_producto['values'] = ["Todos"] + productos_unicos
            self.combo_producto.set("Todos")

    def generar_reporte(self):
        if self.datos_originales is None or self.datos_originales.empty:
            messagebox.showwarning("Atención", "No hay datos cargados para generar un reporte.")
            return

        print("\n" + "="*50)
        print("INICIANDO GENERACIÓN DE REPORTE (DEPURACIÓN)")
        
        try: fecha_inicio = self.cal_inicio.get_date()
        except (tk.TclError, TypeError): fecha_inicio = None
        try: fecha_fin = self.cal_fin.get_date()
        except (tk.TclError, TypeError): fecha_fin = None
        ...
        if not all([agrupar_por, valores, funcion, orden, ordenar_por]):
            messagebox.showwarning("Atención", "Debe seleccionar todas las opciones del reporte.")
            return

        datos_filtrados = aplicar_filtros(self.datos_originales, fecha_inicio, fecha_fin, palabra_clave, producto_seleccionado)
        ...
        reporte_df = generar_reporte_agregado(datos_filtrados, agrupar_por, valores, funcion, orden, ordenar_por)
        
        # Guardar el reporte generado para poder exportarlo después
        self.reporte_actual = reporte_df
    ...
    def mostrar_en_tabla(self, df):
        ...
        if df is None or df.empty:
            messagebox.showinfo("Sin Resultados", "No se encontraron datos con los filtros aplicados.")
            self.tree["columns"] = []
            return
    ...
    def copiar_descripcion(self):
        ...
        if not selected_item:
            messagebox.showwarning("Sin Selección", "Por favor, seleccione un item de la tabla para copiar su descripción.")
            return
        ...
        if item_values:
            ...
            try:
                ...
                messagebox.showinfo("Copiado", f"Se ha copiado al portapapeles:\n\n'{texto_a_copiar}'")
            except tk.TclError:
                messagebox.showerror("Error", "No se pudo acceder al portapapeles.")

    # --- NUEVA FUNCIÓN DE EXPORTACIÓN ---
    def exportar_txt(self):
        """Exporta el reporte actual a un archivo .txt con sangría de 4 espacios."""
        if self.reporte_actual is None or self.reporte_actual.empty:
             messagebox.showwarning("Sin Datos", "Primero debe generar un reporte válido para poder exportarlo.")
             return
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                # Escribir encabezados con 4 espacios de sangría
                headers = list(self.reporte_actual.columns)
                f.write("    " + ", ".join(headers) + "\n")
                
                # Escribir filas con 4 espacios de sangría
                for index, row in self.reporte_actual.iterrows():
                    # Convertir valores a string y unir con comas
                    linea = ", ".join(map(str, row.values))
                    f.write("    " + linea + "\n")
            
            messagebox.showinfo("Exportar", f"Reporte exportado exitosamente a:\n{filepath}")
        except Exception as e:
